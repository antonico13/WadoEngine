...
namespace Wado::Thread {
    ...
    WdThreadHandle WdGetCurrentThreadHandle() {
        ...
        DuplicateHandle(GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(), &handle, NULL, FALSE, DUPLICATE_SAME_ACCESS);
    ...
    WdThreadLocalID WdThreadLocalAllocate() {
        WdThreadLocalID newID = TlsAlloc();
