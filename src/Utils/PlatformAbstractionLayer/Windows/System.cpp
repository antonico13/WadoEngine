...
namespace Wado::System {
    ...
    const std::vector<WdCoreInfo> WdGetAvailableSystemCores() {
        ...
        PSYSTEM_LOGICAL_PROCESSOR_INFORMATION buffer = static_cast<PSYSTEM_LOGICAL_PROCESSOR_INFORMATION>(HeapAlloc(GetProcessHeap(), 0, sizeof(SYSTEM_LOGICAL_PROCESSOR_INFORMATION)));
        ...
        while (seek < length) {
            ...
            if ((sysInfo.Relationship == RelationProcessorCore) && (sysInfo.ProcessorCore.Flags == 1)) {
