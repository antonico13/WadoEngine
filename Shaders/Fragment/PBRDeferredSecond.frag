...
void main() {
    ...
    vec4 diffuseProperties = texture(diffuseSampler, fragTexCoord);
    vec4 specularProperties = texture(specularSampler, fragTexCoord);
    vec4 meshProperties = texture(meshSampler, fragTexCoord);
    ...
    vec3 posCamspace = texture(positionSampler, fragTexCoord).xyz;
